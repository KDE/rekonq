    ...
    , m_info(info)
{
    ...
    if (cert.isNull())
    {
        ...
        imageLabel->setPixmap(QIcon::fromTheme( QL1S("security-low") ).pixmap(c_dim));
        layout->addWidget(label, rows++, 1);
    }
    else
    {
    ...
    else
    {
        ...
        const QStringList cipherInfo = m_info.ciphers().split( QL1C('\n'), QString::KeepEmptyParts);
        label = new QLabel(this);
        label->setWordWrap(true);
        label->setText(
    ...
    imageLabel->setPixmap(QIcon::fromTheme( QL1S("dialog-information") ).pixmap(c_dim));
